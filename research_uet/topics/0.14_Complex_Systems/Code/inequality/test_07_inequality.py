...
def run_test():
    ...
    try:
        ...
        result_dir = Path(__file__).resolve().parent.parent.parent / "Result" / "inequality"
        ...
        if results:
            ...
            fig.add_hline(
                y=1.0, line_dash="dash", line_color="blue", annotation_text="Balanced (k=1)"
            )

            fig.update_layout(
                title="Global Economic Health Index (Top 20)",
                yaxis_title="k-Index (√(P/D) × Emp)",
                xaxis_title="Country",
            )
