...
BETA_MINUS_ISOTOPES = {
    ...
    "C14": {"parent": "C14", "half_life_years": 5730, "Q_value_keV": 156.5, "use": "Dating"},
    "P32": {"parent": "P32", "half_life_days": 14.3, "Q_value_keV": 1711, "use": "Medical therapy"},
    "Sr90": {
        "parent": "Sr90",
    ...
    "Co60": {"parent": "Co60", "half_life_years": 5.27, "Q_value_keV": 2824, "use": "Radiotherapy"},
}
...
def fermi_function(Z, E_e):
    alpha = 1 / 137
    eta = alpha * Z * m_e / E_e
    return 2 * np.pi * eta / (1 - np.exp(-2 * np.pi * eta))


def phase_space_factor(Q):
    return Q**5
...
def uet_lifetime_from_Q(Q_keV, reference_tau=878.4, reference_Q=782.3):
    return reference_tau * (reference_Q / Q_keV) ** 5
